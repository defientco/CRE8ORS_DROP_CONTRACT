...
contract Cre8orsClaimPassportMinter {
    ...
    constructor(address _cre8orsClaimContractAddress, address _cre8orsPassportContractAddress) {
    ...
    function claimPassport(uint256 _tokenId) external returns (uint256) {
        require(IERC721A(cre8orsClaimContractAddress).ownerOf(_tokenId) == msg.sender, "You do not own this token");
        ICre8orsCollective(cre8orsClaimContractAddress).burn(_tokenId);
        // Mint the token to the sender
        return IERC721Drop(cre8orsPassportContractAddress).adminMint(msg.sender, 1);
    }
}
