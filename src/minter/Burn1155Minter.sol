...
contract Burn1155Minter {
    ...
    function purchase(address target, uint256 quantity) external onlyApprovedForAll(target) returns (uint256) {
        IERC1155(_contractInfos[target].burnToken).safeTransferFrom(
            msg.sender, BURN_ADDRESS, 1, _contractInfos[target].burnQuantity * quantity, bytes("")
        );
    ...
    modifier onlyApprovedForAll(address target) {
        if (!IERC1155(_contractInfos[target].burnToken).isApprovedForAll(msg.sender, address(this))) {
