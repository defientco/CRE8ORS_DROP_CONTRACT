...
contract Burn721Minter {
    ...
    event NewMinterInitialized(address indexed target, address burnToken, uint256 burnQuantity);
    ...
    function contractInfos(address target) public view returns (ContractMintInfo memory) {
    ...
    function initializeWithData(address target, bytes memory data) external onlyAdmin(target) {
        (address burnToken, uint256 burnQuantity) = abi.decode(data, (address, uint256));
        _contractInfos[target] = ContractMintInfo({burnToken: burnToken, burnQuantity: burnQuantity});
        emit NewMinterInitialized({target: msg.sender, burnToken: burnToken, burnQuantity: burnQuantity});
    }

    /// @notice mint function
    /// @dev This allows the user to purchase an edition
    /// @dev at the given price in the contract.
    function purchase(address target, uint256 quantity, uint256[] calldata tokensToBurn)
        external
        onlyApprovedForAll(target)
        returns (uint256)
    {
        require(tokensToBurn.length == _contractInfos[target].burnQuantity * quantity, "incorrect burn quantity");

        for (uint256 i = 0; i < tokensToBurn.length; i++) {
            IERC721A(_contractInfos[target].burnToken).safeTransferFrom(msg.sender, BURN_ADDRESS, tokensToBurn[i]);
        }

        uint256 firstMintedTokenId = IERC721Drop(target).adminMint(msg.sender, quantity);
    ...
    modifier onlyApprovedForAll(address target) {
        if (!IERC721A(_contractInfos[target].burnToken).isApprovedForAll(msg.sender, address(this))) {
