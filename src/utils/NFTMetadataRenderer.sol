...
library NFTMetadataRenderer {
    ...
    function createMetadataJSON(
        ...
        bytes memory propertiesString = abi.encodePacked(
            'properties": {"number": ',
            Strings.toString(tokenOfEdition),
            ",",
            traitData(attributeKeys, attributeValues),
            '"name": "',
            name,
            '"}}'
        );
    ...
    /// @notice converts array of keys and values to a metadata string.
    /// @param traitKeys trait keys
    /// @param traitValues trait values
    function traitData(string[] memory traitKeys, string[] memory traitValues)
        internal
        pure
