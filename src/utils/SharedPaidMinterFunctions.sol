...
contract SharedPaidMinterFunctions is ISharedPaidMinterFunctions {
    ...
    function _lockUp(IMinterUtilities.Cart[] memory carts, uint256 startingTokenId) internal {
        ...
        for (uint256 i = 0; i < carts.length; i++) {
            ...
            (uint256 lockupDate, uint256 tierPrice) =
                abi.decode(_getLockUpDateAndPrice(carts[i].tier), (uint256, uint256));
            lockup.setUnlockInfo(cre8orsNFT, tokenId + i, abi.encode(lockupDate, tierPrice));
        }
    }

    function _getLockUpDateAndPrice(uint256 tier) internal view onlyValidTier(tier) returns (bytes memory) {
        IMinterUtilities.TierInfo[] memory tiers =
            abi.decode(IMinterUtilities(minterUtility).getTierInfo(), (IMinterUtilities.TierInfo[]));
