...
contract Cre8orBuilderRewardsTest is DSTest {
    Cre8orBuilderRewards public cre8orsNFTBase;
    ...
    function test_Erc721() public setupCre8orsNFTBase(DEFAULT_EDITION_SIZE) {
        ...
        assertEq(type(uint64).max, cre8orsNFTBase.saleDetails().maxSupply);
    }

    function test_BurnInit() public setupCre8orsNFTBase(DEFAULT_EDITION_SIZE) {
        (address burnToken, uint256 burnQuantity) = cre8orsNFTBase.burnConfig();
        assertEq(BURN_TOKEN, burnToken);
